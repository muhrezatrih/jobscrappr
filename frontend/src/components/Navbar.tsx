  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/80 dark:bg-[#090a0f]/80 backdrop-blur-xl border-b border-zinc-200/80 dark:border-zinc-800/80 transition-colors">
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        {/* Brand Logo - Left Aligned to Container Boundary */}
        <div className="flex items-center">
          ...
          >
            <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-white shadow-sm flex-shrink-0">
              <Sparkles className="w-4 h-4" />
            </div>
            <span className="text-base font-bold whitespace-nowrap">JobFlow Scrappr</span>
          </Link>
        </div>

        {/* Navigation Links - Centered horizontally at exactly 50% */}
        <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center space-x-1 bg-zinc-100/70 dark:bg-zinc-800/50 p-1 rounded-full border border-zinc-200/60 dark:border-zinc-700/60 backdrop-blur-md shadow-sm">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all whitespace-nowrap ${
                  active
        ...
        {/* Right Action: Theme Toggle - Right Aligned to Container Boundary */}
